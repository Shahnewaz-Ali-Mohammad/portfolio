...
function ProjectVisual({ project }: { project: Project }) {
  ...
  return <div className="project-visual">
    <div className="project-image-deck">{project.images.map((image, index) => <img className={`project-carousel-image${index === active ? " active" : ""}`} src={image} alt={`${project.title} interface view ${index + 1} of ${project.images.length}`} aria-hidden={index !== active} key={image} />)}</div>
...
function HeroShowcase() {
  ...
  return <div className="hero-showcase" aria-label="Selected project previews">
    <div className="browser-window">
      ...
      <div className="browser-image-deck">{projects.map((project, index) => <img className={`browser-carousel-image${index === active ? " active" : ""}`} src={project.src} alt={`${project.label} project preview`} aria-hidden={index !== active} key={project.src} />)}</div>
    </div>
...
export default function Home() {
  ...
  return <main>
    <header className="site-header"><a className="brand" href="#top" aria-label="Home"><img className="brand-mark" src="/logo-mark.svg" alt="" width="38" height="38" /><strong>SHAHNEWAZ ALI MOHAMMAD</strong></a><nav aria-label="Primary navigation"><a href="#experience">Experience</a><a href="#work">Work</a><a href="#skills">Skills</a></nav></header>
